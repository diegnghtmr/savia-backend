...
async function createApplication(
  create: CurrencyExchangePort['create'] = vi.fn().mockResolvedValue({
    kind: 'created',
    transfer: TRANSFER,
  } satisfies CurrencyExchangeCreateOutcome),
): Promise<{
  application: NestFastifyApplication;
  create: CurrencyExchangePort['create'];
}> {
  const moduleRef = await Test.createTestingModule({
    ...
    .useValue(verifier)
    .overrideProvider(CURRENCY_EXCHANGE_PORT)
    .useValue({ create })
    .compile();
...
describe('POST /v1/currency-exchanges', () => {
  ...
  it('answers 400 when X-Workspace-Id header is missing', async () => {
    const create = vi.fn<CurrencyExchangePort['create']>();
    const { application } = await createApplication(create);
    const response = await postCurrencyExchange(
      application,
      VALID_CREATE_BODY,
  ...
  it('answers 422 problem+json when request body fails validation (e.g. self-exchange)', async () => {
    const create = vi.fn<CurrencyExchangePort['create']>();
    const { application } = await createApplication(create);
    const response = await postCurrencyExchange(
      application,
