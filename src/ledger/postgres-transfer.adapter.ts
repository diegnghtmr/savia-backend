...
import type { CreateTransferCommand, Transfer } from './transfer.port.js';
import type {
...
export class PostgresTransferAdapter implements TransferStore {
  ...
  public async lockAndReadAccounts(
    ...
    // (e.g. Account A -> Account B and Account B -> Account A) would otherwise acquire locks in opposite
    // order (A then B vs B then A) and deadlock if both transactions run concurrently.
...
returning
  ...
  version`;
    ...
    const transferValues = [
      ...
      null, // exchangeRate (same currency transfer)
      null, // referenceRate (same currency transfer)
      command.occurredAt,
      'confirmed',
    ...
    if (!row) {
      throw new Error('Created transfer row could not be read.');
    }

    // 2. Insert balanced pair of ledger postings with transfer_id set and transaction_id NULL
    // Source leg: debit/outflow from source account (negated amount)
    // Destination leg: credit/inflow to destination account (positive amount)
    const postingsSql = `
...
values
  ...
  ($1::uuid, $2::uuid, null, $7::uuid, 'account', $8, $5, 'confirmed', $6)`;
    ...
    // 3. Enforce deferred constraints (balanced-postings check) before commit
